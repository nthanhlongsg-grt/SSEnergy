...
description: >-
  Guides implementation of SSE's internal web app for solar inverter repair
  ...
  designing schema, APIs, UI, permissions, or features for the SSE project,
...
# SSE — Hệ thống quản lý workflow sửa chữa biến tần

Dự án nội bộ **Công ty SSE**: web app theo dõi toàn bộ quy trình sửa chữa biến tần (inverter) năng lượng mặt trời, **bảo hành sau sửa chữa**, tài chính nội bộ, dịch vụ hiện trường, và kho. Agent phải giữ nhất quán thuật ngữ, entity, luồng trạng thái, và ma trận quyền dưới đây khi viết code, migration, hoặc tài liệu kỹ thuật.
...
App **nội bộ SSE** (không phải Portal KH) dùng **layout dashboard hiện đại** giống mockup **điều phối / quản trị** (sidebar + header + vùng nội dung trong **card** trắng, bảng dữ liệu rõ cột, **badge** màu pastel). Tham chiếu trực quan: ảnh mockup team cung cấp (vd. lưu dưới `assets/` trong repo hoặc Figma).

### Bố cục 3 vùng (desktop)

| Vùng | Hành vi | Gợi ý component (Ant Design / MUI / tương đương) |
|------|---------|--------------------------------------------------|
| **Sidebar trái** | Cố định (hoặc thu gọn); **logo SSE** + menu nhóm (**HỆ THỐNG**, **QUẢN LÝ**, …). Mục đang chọn: nền xanh nhạt, icon + chữ **primary**. Mục có con: **chevron** + SubMenu / collapse. | `Layout.Sider` + `Menu`; hoặc `Drawer` trên mobile thay sidebar. |
...
3. **Hợp đồng cần thực hiện** — `contracts` có **`owner_user_id`** = user (hoặc quy tắc SSE mở rộng), `status` ∈ {`signed`, `in_progress`, …} và (tuỳ nghiệp vụ) còn việc: chưa đủ thu, chưa gắn phiếu, v.v. Hiển thị **deadline** nếu có cột **`due_at` / `expected_close_date`** trên HĐ (bổ sung schema nếu cần).
...
- **Bảo hành sau sửa chữa:** mỗi lần hoàn tất sửa chữa (bàn giao), biến tần có thể có **kỳ bảo hành theo phiếu** (thời hạn, phạm vi). Lưu lịch sử theo từng `service_case` / `device`; phiếu mới trong hạn có thể **liên kết bảo hành cũ** hoặc gia hạn theo policy SSE.
...
| Hợp đồng | `contract`, `quote`, `contract_line` | Trạng thái: draft → signed → in_progress → closed / cancelled. **`contract_kind`** phân loại HĐ: **dịch vụ sửa chữa** (một máy, hai loại phí) **hoặc** **mua bán thiết bị** — **mua vào** (SSE mua từ NCC) / **bán ra** (SSE bán cho KH). Chi tiết mục *Loại hợp đồng & dòng HĐ* bên dưới. **`owner_user_id`** (người phụ trách HĐ) để **Dashboard** và **thông báo** HĐ. |
...
| **`equipment_purchase`** | **Mua vào** thiết bị (SSE mua từ NCC) | **`supplier_id`** (FK `suppliers`); `customer_id` thường NULL | Mỗi dòng = **hàng mua**: tham chiếu **`part_id`** (SKU `part_kind` thường là `inverter` / `board` tùy danh mục), `quantity`, `unit_price`, `line_total`; **`line_fee_kind`** không dùng (NULL + validate ứng dụng). |
| **`equipment_sale`** | **Bán ra** thiết bị (SSE bán cho KH) | `customer_id`; optional `site_id` | Mỗi dòng = **hàng bán**: `part_id`, `quantity`, đơn giá; optional **serial** từng dòng nếu bán máy có SN (`serial_number` trên dòng hoặc gắn `equipment_id` / `device_id` sau giao). **`line_fee_kind`** không dùng cho loại này. |
...
**Mục tiêu:** Màn hình và API **báo cáo — thống kê tài chính** cho kế toán / lãnh đạo: so sánh **thu**, **chi**, và các chỉ số **công nợ / backlog** (tuỳ định nghĩa SSE) trong **từng tháng dương lịch** và **từng quý** (Q1–Q4). Nhãn và format số qua **i18n**.
...
- **Thu − chi (gộp):** chỉ hiển thị khi SSE chấp nhận định nghĩa; ghi chú disclaimer trên UI.  
...
- **Bắt đầu:** ngày hiệu lực (thường = ngày `delivered` hoặc `closed` — chốt một quy tắc SSE và giữ nhất quán).
- **Kết thúc:** `warranty_end_date` hoặc `warranty_duration_months` + tính toán; hỗ trợ cảnh báo **sắp hết hạn** / **đã hết hạn** (query/UI).
- **Nội dung:** loại bảo hành (ví dụ: linh kiện thay, công lao động), điều kiện loại trừ (tuỳ policy), ghi chú; có thể tham chiếu `contract_line` (M1) nếu hợp đồng quy định thời hạn chuẩn. **Spare:** khai báo **`repair_warranty_covered_part`** (danh sách part trong phạm vi BH) khi nghiệp vụ SSE yêu cầu — đối chiếu với xuất kho có `repair_warranty_id`.
...
Các nhánh hủy: `cancelled` từ `diagnosing` hoặc `waiting_parts` (hoặc theo policy SSE — ghi rõ trong code comment nếu mở rộng).
...
Màn **chi tiết một phiếu** (`service_case`) bố cục theo dạng **ticket** (như mockup nội bộ SSE): header + lưới thẻ thông tin + khu vực mô tả/đính kèm + nhật ký & comment. Toàn bộ nhãn UI qua **i18n** (`vi` / `en`).
...
**5. Bổ sung bắt buộc so với mockup cơ bản (domain SSE)**
...
| `admin` | Truy cập **cả ba** module nghiệp vụ (M1, M2, M3) theo policy công ty; không tự động đồng nghĩa bypass mọi quy tắc phê duyệt trừ khi spec ghi rõ. **Chat:** có quyền **`chat.conversation:read_all`** — **xem mọi cuộc trò chuyện** (kể cả không tham gia), kèm UI giám sát / audit theo policy SSE. |
...
| **`chat.conversation:read_all`** | **Giám sát:** đọc **mọi cuộc** trong hệ thống, **không cần** là thành viên. Gán cho role **`admin`** (và có thể **`dev`**); **bắt buộc** ghi **audit log** khi mở/xem cuộc qua quyền này nếu SSE yêu cầu tuân thủ / minh bạch. |
...
**Audit:** mỗi lần user có **`read_all`** mở thread hoặc export — ghi `audit_logs` (`action` = `chat_supervisor_view`, `entity_id` = `conversation_id`) nếu policy SSE yêu cầu.
...
|---------|-----------|----------------|
| **Hợp đồng** | Hợp đồng có `customer_id` khớp user: mã, **`contract_kind`**, trạng thái, thiết bị/site liên quan (nếu có). Với **`repair_service`:** **tóm tắt** phí sửa vs phí BH. Với **`equipment_sale`:** tóm tắt **hàng bán** (model/SKU, SL — tùy policy hiển thị). | HĐ **`equipment_purchase`** (mua vào NCC), ghi chú nội bộ, giá vốn chi tiết nếu SSE ẩn với KH. |
...
- API và UI portal: `WHERE is_internal = false` (và chỉ `activity_type` phù hợp — vd. `comment`, `status_change` nếu SSE cho phép KH thấy đổi trạng thái).
...
- [ ] Sửa trong thời hạn BH: movement linh kiện có `repair_warranty_id`; khi tạo BH có **bổ sung** phạm vi spare (covered parts) nếu policy SSE yêu cầu.  
